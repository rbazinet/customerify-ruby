...
[![Build Status](https://secure.travis-ci.org/simplemailr/simplemailr-ruby.png?branch=master)](http://travis-ci.org/simplemailr/simplemailr-ruby)
...
through the [Simplemailr JavaScript snippet](http://simplemailr.com/docs/basic-integration.html).
...
If you're using Rails, create an initializer `config/initializers/simplemailr.rb`:

    simplemailr = simplemailr::Client.new("YOUR SITE ID", "YOUR API SECRET KEY")
...
encourage your customers to perform an action.
    ...
    simplemailr.track(5, "purchase", :type => "socks", :price => "13.99")
...
(in seconds since the epoch), and we'll use that as the date the event occurred.
    ...
    simplemailr.track(5, "purchase", :type => "socks", :price => "13.99", :timestamp => 1365436200)

## Contributing

1. Fork it
2. Clone your fork (`git clone git@github.com:MY_USERNAME/simplemailr-ruby.git && cd simplemailr-ruby`)
